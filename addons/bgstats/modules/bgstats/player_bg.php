 ...
 */
	...
	$sqlpartial .= " and `cb`.char_id = ?";
	$bind = array_merge($bind, (array)$player_id);
	
	$sql  = "SELECT cb.*,c.*,g.name as g_name FROM {$server->charMapDatabase}.`char_bg` AS cb left join {$server->charMapDatabase}.`char` as c on `c`.char_id=`cb`.char_id left join {$server->charMapDatabase}.`guild` as g on `c`.guild_id=`g`.guild_id left join login ON `login`.account_id =`c`.account_id {$sqlpartial}";
	$sth  = $server->connection->getStatement($sql);
	
	$sth->execute($bind);
	$profile = $sth->fetchAll();

	$title = $profile[0]->name." (BG Profile)";
	
	$bind     = array();

	$col  = "`char`.char_id,`char`.sex, `char`.name AS char_name, `char`.class AS char_class, cb.points,";
	$col .= "`char`.guild_id, guild.name AS guild_name, guild.emblem_len AS guild_emblem_len";


	$sql  = "SELECT $col FROM {$server->charMapDatabase}.`char_bg` AS cb ";
	$sql .= "LEFT JOIN {$server->charMapDatabase}.`char` ON `char`.char_id = cb.char_id ";
	...
	$sql .= "ORDER BY cb.points DESC, cb.points DESC ";
	$sql .= "LIMIT 99999";
	$sth  = $server->connection->getStatement($sql);
