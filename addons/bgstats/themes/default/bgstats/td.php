...
<h2>Battleground <?php echo Flux::message('BGTdlabel') ?></h2>
<?php if ($chars): ?>
...
<table class="horizontal-table">
	<tr>
		...
		<th><?php echo $paginator->sortableColumn('td_kills',htmlspecialchars(Flux::message('TdKill'))) ?></th>
		<th><?php echo $paginator->sortableColumn('td_deaths',htmlspecialchars(Flux::message('TdDeath'))) ?></th>
	</tr>
	...
	<tr<?php if (!isset($char)) echo ' class="empty-row"'; if ($i === 0) echo ' class="top-ranked" title="<strong>'.htmlspecialchars($char->name).'</strong> '.htmlspecialchars(Flux::message('TopRankedLabel')).' '.$topRankType.'!"' ?>>
		...
		<?php endif ?>
		<td><?php echo number_format($char->td_wins) ?></td>
		<td><?php echo number_format($char->td_lost) ?></td>
		<td><?php echo number_format($char->td_tie) ?></td>
		<td><?php echo number_format($char->td_kills) ?></td>
		<td><?php echo number_format($char->td_deaths) ?></td>
		<?php else: ?>
		<td colspan="8"></td>
		<?php endif ?>
	</tr>
