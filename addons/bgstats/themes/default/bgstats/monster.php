...
<h2>Battleground <?php echo Flux::message('Monsterlabel') ?></h2>
<?php if ($chars): ?>
...
<table class="horizontal-table">
	<tr>
		...
		<th><?php echo $paginator->sortableColumn('gstone_kill',htmlspecialchars(Flux::message('GStoneKill'))) ?></th>
		<th><?php echo $paginator->sortableColumn('barricade_kill',htmlspecialchars(Flux::message('BarricadeKill'))) ?></th>
	...
	<tr<?php if (!isset($char)) echo ' class="empty-row"'; if ($i === 0) echo ' class="top-ranked" title="<strong>'.htmlspecialchars($char->name).'</strong> '.htmlspecialchars(Flux::message('TopRankedLabel')).' '.$topRankType.'!"' ?>>
		...
		<td><?php echo number_format($char->gstone_kill) ?></td>
		<td><?php echo number_format($char->barricade_kill) ?></td>
