...
<h2>Battleground <?php echo Flux::message('BGSclabel') ?></h2>
<?php if ($chars): ?>
...
<table class="horizontal-table">
	<tr>
		...
		<th><?php echo $paginator->sortableColumn('sc_captured',htmlspecialchars(Flux::message('ScCaptured'))) ?></th>
		<th><?php echo $paginator->sortableColumn('sc_stolen',htmlspecialchars(Flux::message('ScStolen'))) ?></th>
		<th><?php echo $paginator->sortableColumn('sc_droped',htmlspecialchars(Flux::message('ScDroped'))) ?></th>
	</tr>
	...
	<tr<?php if (!isset($char)) echo ' class="empty-row"'; if ($i === 0) echo ' class="top-ranked" title="<strong>'.htmlspecialchars($char->name).'</strong> '.htmlspecialchars(Flux::message('TopRankedLabel')).' '.$topRankType.'!"' ?>>
		...
		<?php endif ?>
		<td><?php echo number_format($char->sc_wins) ?></td>
		<td><?php echo number_format($char->sc_lost) ?></td>
		<td><?php echo number_format($char->sc_tie) ?></td>
		<td><?php echo number_format($char->sc_captured) ?></td>
		<td><?php echo number_format($char->sc_stolen) ?></td>
		<td><?php echo number_format($char->sc_droped) ?></td>
		<?php else: ?>
		<td colspan="8"></td>
		<?php endif ?>
	</tr>
