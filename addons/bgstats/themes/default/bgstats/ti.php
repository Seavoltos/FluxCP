...
<h2>Battleground <?php echo Flux::message('BGTilabel') ?></h2>
<?php if ($chars): ?>
...
<table class="horizontal-table">
	<tr>
		...
		<th><?php echo $paginator->sortableColumn('skulls',htmlspecialchars(Flux::message('TiSkulls'))) ?></th>
	</tr>
	...
	<tr<?php if (!isset($char)) echo ' class="empty-row"'; if ($i === 0) echo ' class="top-ranked" title="<strong>'.htmlspecialchars($char->name).'</strong> '.htmlspecialchars(Flux::message('TopRankedLabel')).' '.$topRankType.'!"' ?>>
		...
		<?php endif ?>
		<td><?php echo number_format($char->ti_wins) ?></td>
		<td><?php echo number_format($char->ti_lost) ?></td>
		<td><?php echo number_format($char->ti_tie) ?></td>
		<td><?php echo number_format($char->skulls) ?></td>
		<?php else: ?>
		<td colspan="8"></td>
		<?php endif ?>
	</tr>
