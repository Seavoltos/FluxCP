...
<h2>Battleground <?php echo Flux::message('BGBosslabel') ?></h2>
<?php if ($chars): ?>
...
<table class="horizontal-table">
	<tr>
		...
		<th><?php echo $paginator->sortableColumn('boss_killed',htmlspecialchars(Flux::message('BossKilled'))) ?></th>
		<th><?php echo $paginator->sortableColumn('boss_damage',htmlspecialchars(Flux::message('BossDamage'))) ?></th>
		<th><?php echo $paginator->sortableColumn('boss_flags',htmlspecialchars(Flux::message('BossFlags'))) ?></th>
	</tr>
	...
	<tr<?php if (!isset($char)) echo ' class="empty-row"'; if ($i === 0) echo ' class="top-ranked" title="<strong>'.htmlspecialchars($char->name).'</strong> '.htmlspecialchars(Flux::message('TopRankedLabel')).' '.$topRankType.'!"' ?>>
		...
		<?php endif ?>
		<td><?php echo number_format($char->boss_wins) ?></td>
		<td><?php echo number_format($char->boss_lost) ?></td>
		<td><?php echo number_format($char->boss_tie) ?></td>
		<td><?php echo number_format($char->boss_killed) ?></td>
		<td><?php echo number_format($char->boss_damage) ?></td>
		<td><?php echo number_format($char->boss_flags) ?></td>
		<?php else: ?>
		<td colspan="8"></td>
		<?php endif ?>
	</tr>
