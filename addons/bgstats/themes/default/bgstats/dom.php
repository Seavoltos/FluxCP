...
<h2>Battleground <?php echo Flux::message('BGDomlabel') ?></h2>
<?php if ($chars): ?>
...
<table class="horizontal-table">
	<tr>
		...
		<th><?php echo $paginator->sortableColumn('dom_bases',htmlspecialchars(Flux::message('DomBases'))) ?></th>
		<th><?php echo $paginator->sortableColumn('dom_off_kills',htmlspecialchars(Flux::message('DomOffKill'))) ?></th>
		<th><?php echo $paginator->sortableColumn('dom_def_kills',htmlspecialchars(Flux::message('DomDefKill'))) ?></th>
	</tr>
	...
	<tr<?php if (!isset($char)) echo ' class="empty-row"'; if ($i === 0) echo ' class="top-ranked" title="<strong>'.htmlspecialchars($char->name).'</strong> '.htmlspecialchars(Flux::message('TopRankedLabel')).' '.$topRankType.'!"' ?>>
		...
		<?php endif ?>
		<td><?php echo number_format($char->dom_wins) ?></td>
		<td><?php echo number_format($char->dom_lost) ?></td>
		<td><?php echo number_format($char->dom_tie) ?></td>
		<td><?php echo number_format($char->dom_bases) ?></td>
		<td><?php echo number_format($char->dom_off_kills) ?></td>
		<td><?php echo number_format($char->dom_def_kills) ?></td>
		<?php else: ?>
		<td colspan="8"></td>
		<?php endif ?>
	</tr>
