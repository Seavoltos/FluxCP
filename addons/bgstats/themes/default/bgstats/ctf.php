...
<h2>Battleground <?php echo Flux::message('BGCtflabel') ?></h2>
<?php if ($chars): ?>
...
<table class="horizontal-table">
	<tr>
		...
		<th><?php echo $paginator->sortableColumn('ctf_captured',htmlspecialchars(Flux::message('CtfCaptured'))) ?></th>
		<th><?php echo $paginator->sortableColumn('ctf_taken',htmlspecialchars(Flux::message('CtfTaken'))) ?></th>
		<th><?php echo $paginator->sortableColumn('ctf_droped',htmlspecialchars(Flux::message('CtfDroped'))) ?></th>
	</tr>
	...
	<tr<?php if (!isset($char)) echo ' class="empty-row"'; if ($i === 0) echo ' class="top-ranked" title="<strong>'.htmlspecialchars($char->name).'</strong> '.htmlspecialchars(Flux::message('TopRankedLabel')).' '.$topRankType.'!"' ?>>
		...
		<?php endif ?>
		<td><?php echo number_format($char->ctf_wins) ?></td>
		<td><?php echo number_format($char->ctf_lost) ?></td>
		<td><?php echo number_format($char->ctf_tie) ?></td>
		<td><?php echo number_format($char->ctf_captured) ?></td>
		<td><?php echo number_format($char->ctf_taken) ?></td>
		<td><?php echo number_format($char->ctf_droped) ?></td>
		<?php else: ?>
		<td colspan="8"></td>
		<?php endif ?>
	</tr>
