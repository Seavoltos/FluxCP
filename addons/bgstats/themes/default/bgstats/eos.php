...
<h2>Battleground <?php echo Flux::message('BGEoslabel') ?></h2>
<?php if ($chars): ?>
...
<table class="horizontal-table">
	<tr>
		...
		<th><?php echo $paginator->sortableColumn('eos_bases',htmlspecialchars(Flux::message('EosBases'))) ?></th>
		<th><?php echo $paginator->sortableColumn('eos_flags',htmlspecialchars(Flux::message('EosFlags'))) ?></th>
	</tr>
	...
	<tr<?php if (!isset($char)) echo ' class="empty-row"'; if ($i === 0) echo ' class="top-ranked" title="<strong>'.htmlspecialchars($char->name).'</strong> '.htmlspecialchars(Flux::message('TopRankedLabel')).' '.$topRankType.'!"' ?>>
		...
		<?php endif ?>
		<td><?php echo number_format($char->eos_wins) ?></td>
		<td><?php echo number_format($char->eos_lost) ?></td>
		<td><?php echo number_format($char->eos_tie) ?></td>
		<td><?php echo number_format($char->eos_bases) ?></td>
		<td><?php echo number_format($char->eos_off_kills) ?></td>
		<?php else: ?>
		<td colspan="8"></td>
		<?php endif ?>
	</tr>
