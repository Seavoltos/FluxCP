...
<h2><?php echo Flux::message('WoEOveralllabel') ?></h2>
<?php if ($chars): ?>
...
<table class="horizontal-table">
	<tr>
		...
		<th><?php echo $paginator->sortableColumn('zeny_used',Zeny) ?></th>
		<th><?php echo $paginator->sortableColumn('hp_heal_potions',HP) ?><p><img src="./addons/bgstats/images/item/small/504.png"></p></th>
		<th><?php echo $paginator->sortableColumn('sp_heal_potions',SP) ?><p><img src="./addons/bgstats/images/item/small/505.png"></p></th>
	</tr>
	...
	<tr<?php if (!isset($char)) echo ' class="empty-row"'; if ($i === 0) echo ' class="top-ranked" title="<strong>'.htmlspecialchars($char->name).'</strong> '.htmlspecialchars(Flux::message('TopRankedLabel')).' '.$topRankType.'!"' ?>>
		...
		<?php if (isset($char)): ?>
		<td>
			<a title="Ver estadísticas" href="<?=$this->url('bgstats', 'player_woe', array('player_id' => $char->char_id))?>">
		...
		<td><strong>
			...
			<?php else: ?>
				<a title="Ver estadísticas" href="<?=$this->url('bgstats', 'player_woe', array('player_id' => $char->char_id))?>"><?php echo htmlspecialchars($char->name) ?></a>
			<?php endif ?>
		</strong></td>
		...
		<?php endif ?>
		<td><?php echo number_format($char->top_damage) ?></td>
		<td><?php echo number_format($char->damage_done) ?></td>
		<td><?php echo number_format($char->damage_received) ?></td>
		<td><?php echo number_format($char->kill_count) ?></td>
		<td><?php echo number_format($char->death_count) ?></td>
		...
		<td><?php echo number_format($char->hp_heal_potions) ?></td>
		<td><?php echo number_format($char->sp_heal_potions) ?></td>
		<?php else: ?>
		<td colspan="8"></td>
		<?php endif ?>
	</tr>
